...
/* ---------- ground bake: a pan cannot invalidate a bake that holds the whole plot ----------
   The edit path got a settle; the pan path got nothing, so `panDev>=MD` re-baked
   the instant the camera left the baked margin. Simulated on a real 70x39
   garden, ONE SECOND of ordinary mouse-drag panning forced 2 bakes at a slow
   drag, 6 at a normal one and 10 at a fast one, each ~30ms.

   Nearly all of it changed nothing: the bake window is clamped to the plot, so
   once the whole plot is inside it there is no further ground anywhere. Verified
   in a browser rather than argued — with the bake frozen outright and the camera
   panned 600 device px (3x the 200px margin) the frame was pixel-identical to
   one re-baking freely, 0% of pixels differing against 0% for the control.

   render() needs a real canvas, so the policy is extracted and tested directly,
   exactly as groundEditThrottled is above. */

test('panning a fully-baked plot stops forcing bakes; a partial bake still does', () => {
  setup(21, 21);
  const MD = 200;
  const pan = (complete) => {
    groundBakeComplete = complete;
    let bakes = 0, camAt = 0;                     // device px travelled since the bake
    for (let f = 0; f < 60; f++){                 // one second of drag-panning
      camAt += 15;                                // ~900 px/s, an ordinary drag
      if (groundPanForcesBake(camAt, MD, false)){ bakes++; camAt = 0; }
    }
    return bakes;
  };
  assertEqual(pan(false), 4, 'a partial bake still re-bakes every time the camera leaves the margin');
  assertEqual(pan(true), 0, 'a complete bake rides the whole pan — there is no more ground to draw');
});

test('a bake is complete only with a full pad of skirt beyond the plot', () => {
  setup(21, 21);                                  // GW = GH = 21
  const pad = 5;
  /* The trap this exists for: the CLAMPED, PADDED window reports 0..GW-1 as soon
     as `pad` alone carries it there, which happens even when the plot runs off
     the edge of the canvas and is being clipped. Judging completeness on that
     would skip the pan re-bake and slide a clipped edge into view. */
  assert(!bakeCoversWholePlot(0, GW - 1, 0, GH - 1, pad),
    'reaching exactly the plot edges is NOT complete — nothing is drawn beyond them');
  assert(!bakeCoversWholePlot(-2, GW + 1, -2, GH + 1, pad),
    'nor is a skirt narrower than the pad');
  assert(bakeCoversWholePlot(-pad, GW - 1 + pad, -pad, GH - 1 + pad, pad),
    'a full pad of skirt on every side is complete');
  assert(bakeCoversWholePlot(-40, GW + 40, -40, GH + 40, pad), 'and so is more than that');
  // each side independently, so a transposed or dropped comparison is caught
  assert(!bakeCoversWholePlot(0, GW - 1 + pad, -pad, GH - 1 + pad, pad), 'left edge short');
  assert(!bakeCoversWholePlot(-pad, GW - 1, -pad, GH - 1 + pad, pad), 'right edge short');
  assert(!bakeCoversWholePlot(-pad, GW - 1 + pad, 0, GH - 1 + pad, pad), 'top edge short');
  assert(!bakeCoversWholePlot(-pad, GW - 1 + pad, -pad, GH - 1, pad), 'bottom edge short');
});

test('a zoom in flight still forces the pan bake', () => {
  setup(21, 21);
  groundBakeComplete = true;
  /* At k !== 1 the blit RESAMPLES the baked canvas, so "the bake holds every
     tile" stops being a sufficient answer — the pixels themselves are wrong
     scale. The zoom disjunct owns that case; this one must not swallow it. */
  assert(groundPanForcesBake(300, 200, true), 'complete bake + zoom in flight still bakes');
  assert(!groundPanForcesBake(300, 200, false), 'complete bake + pure pan does not');
  assert(!groundPanForcesBake(199, 200, false), 'inside the margin never bakes');
  assert(!groundPanForcesBake(199, 200, true), 'not even mid-zoom — the margin is the gate');
  groundBakeComplete = false;
});

/* The saving comes from suppressing the MID-GESTURE forced bake only. The
   GROUND_PAN_SETTLE bake must survive, or a resting frame is left showing the
   blit's half-pixel rounding instead of a fresh rasterization — and no unit
   test above can see that, because they all call the policy directly. Pin it on
   the source, the way the sw.js skipWaiting test does. */
test('the pan-settle bake survives the pan guard', () => {
  const src = readRepoFile('js/renderer.js').replace(/\/\*[\s\S]*?\*\//g, '');
  const body = src.slice(src.indexOf('const mustBake ='), src.indexOf('if (mustBake){'));
  assert(/groundPanForcesBake\(\s*panDev\s*,\s*MD\s*,\s*zoomStale\s*\)/.test(body),
    'the margin test goes through the guard rather than a bare panDev>=MD');
  assert(/camStale\s*&&\s*!zoomStale\s*&&\s*t-groundCamT>GROUND_PAN_SETTLE/.test(body),
    'and the settle bake after the camera stops is still there');
});

/* The unit test above pins what bakeCoversWholePlot decides. This pins what it
   is HANDED, which is the actual trap and which no unit test can see: passing
   the clamped, padded window (bx0..by1) instead of the raw corner range makes
   every bake report itself complete, because the clamp forces exactly the
   values the function is looking for. Verified by making that substitution on
   purpose — the whole suite still passed until this test existed. */
test('completeness is judged on the raw corner range, not the clamped window', () => {
  const src = readRepoFile('js/renderer.js').replace(/\/\*[\s\S]*?\*\//g, '');
  const bake = src.slice(src.indexOf('if (mustBake){'), src.indexOf('paintGround(groundCtx'));

  assert(/groundBakeComplete\s*=\s*bakeCoversWholePlot\(\s*rx0\s*,\s*rx1\s*,\s*ry0\s*,\s*ry1\s*,\s*pad\s*\)/.test(bake),
    'the raw range and the real pad are what get measured');
  assert(!/bakeCoversWholePlot\(\s*bx0/.test(bake),
    'never the clamped window — the clamp manufactures a passing answer');
  assert(!/bakeCoversWholePlot\([^)]*,\s*0\s*\)/.test(bake),
    'and never with the pad zeroed out');

  // ...and the raw range really is raw: no Math.max(0,…) / Math.min(GW-1,…)
  assert(/const rx0=Math\.min\(bc/.test(bake) && /const ry0=Math\.min\(bc/.test(bake),
    'rx0/ry0 are the bare minimum of the four corners');
  assert(/const rx1=Math\.max\(bc/.test(bake) && /const ry1=Math\.max\(bc/.test(bake),
    'rx1/ry1 are the bare maximum');
  // the clamped window still exists and is still what paintGround is given
  assert(/const bx0=Math\.max\(0,rx0-pad\)/.test(bake) && /const bx1=Math\.min\(GW-1,rx1\+pad\)/.test(bake),
    'the clamped window is still derived for the bake itself');
});
