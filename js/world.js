...
// the directions this tile actually falls away in — where a wall has a face
function elevationDropDirs(x,y){
  const h=elevationAt(x,y), out=[];
  ELEV_DIRS.forEach(([dx,dy],i)=>{
...
/* ---------- the season wash ----------
   ...
   fallback if a browser ever regresses gradient reuse — it is ~25 lines and
   buys a free comparison in a system that has now mismeasured itself twice.
