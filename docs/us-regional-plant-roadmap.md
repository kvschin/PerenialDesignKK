...
- [x] **Offer catalog-supported zones and resolve ZIPs without silent clamping.** The catalog-derived zone controls landed in 0.8.66 (currently 2–11). The 0.8.67 follow-up replaces broad prefix guesses with 40,502 published five-digit ZIP/half-zone pairs, bundled offline, and adds a user-initiated official USDA-map link. Setup reports unsupported results without rounding them into catalog coverage. Winter captions show temperature intervals; saved gardens keep their existing zones. See [source, behavior, and verification](zone-lookup.md). This completes the initial zone follow-up; it does not add new tropical plant coverage or property-level climate inference. [USDA guidance](https://planthardiness.ars.usda.gov/pages/how-to-use-the-maps)
