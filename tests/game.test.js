...
test('Florida plants are reachable by category, exact discovery, and botanical synonyms', () => {
  setup();
  const library=new Set(libraryAllKeys()),refs=new Set(allPlantRefs().map(r=>plantRefId(r)));
  for(const key of floridaKeys){
    assert(library.has(key)&&libraryCatFor(key),`${key}: reachable library category`);
    assert(refs.has(key+'|'),`${key}: exact discovery reference`);
    for(const synonym of PLANTS[key].synonyms||[]){
      const q=synonym.toLowerCase();
  ...
  assertEqual(libraryCatFor('elliottslovegrass').id,'grasses','Elliott lovegrass stays in grasses');
...
test('Florida morphology stays finite, deterministic, and distinct in both renderers', () => {
  function trace(key,season,seed=307){
    const ops=[],ctx=new Proxy({}, {
      get(o,p){
        if(p in o)return o[p];
        if(p==='createLinearGradient'||p==='createRadialGradient')return ()=>({addColorStop(){}});
  ...
  try { for(const mode of [false,true]){
    ART2.on=mode;
    for(const key of floridaKeys)for(const season of SEASONS)
      assertEqual(trace(key,season),trace(key,season),`${key} ${season}: stable seed`);
    assert(trace('coontie','Summer')!==trace('coontie','Summer',308),'cycad fronds vary with the stable plant seed');
    assert(trace('sawpalmetto','Summer')!==trace('sawpalmetto','Summer',308),'fan-palm ramets vary with the stable plant seed');
...
test('a named palette All view spans its saved plant categories', () => {
  ...
  /* discoveryCategoryCounts is what the counted category strip actually calls
     (ui.js) — the palette-only counter this used to ask was the dead category
     dropdown's, and went with it in 0.8.70. */
  const data = discoveryCategoryCounts(activeDiscovery());
...
/* ---------- the category dropdown is gone ----------
   `.cat-current` was the only thing in the app that could set catMenuOpen, and
   it was display:none in BOTH responsive tiers — DOCK and SHEET, which are
   exact complements — so no viewport could open the popover it controlled. The
   counted category strip does that job at every breakpoint.

   It was not inert while it sat there: the popover's plants section read a null
   discovery lens, so any programmatic route in threw, and that took down
   verifyTrayCache() — the guard against a stale catalog — in all three of its
   configurations. Fixed in 0.8.69; removed in 0.8.70, the fix having shown the
   code was reached by nothing but its own tests.

   A grep, because what this prevents is somebody re-adding a trigger without
   noticing the popover it opens was deleted out from under it. */
test('no dead category dropdown is left behind', () => {
  const js = ['js/tray.js', 'js/world.js', 'js/input.js', 'js/view.js']
    .map(readRepoFile).join('\n');
  for (const dead of ['catMenuOpen', 'cat-current', 'cat-pop', 'catalogCategoryMenu',
                      'discoveryAllCategoryLabel', 'discoveryCollectionCategoryData'])
    assertEqual(js.includes(dead), false, `js/ no longer mentions ${dead}`);
  const css = readRepoFile('styles.css');
  for (const dead of ['.cat-current', '.cat-pop'])
    assertEqual(css.includes(dead), false, `styles.css no longer styles ${dead}`);

  // and what replaced it is still there, still choosing a category the same way
  const tray = readRepoFile('js/tray.js');
